...
- **多模型接入** — 不同 Agent 可配置不同模型（如 Maker 用 DeepSeek、Checker 用 SenseNova），避免同模型幻觉问题
...
不同 Agent 可以使用不同的模型，适用于 Maker-Checker 等场景——避免同一模型既当裁判又当选手导致的幻觉问题。

在 `.env` 中添加模型 profile 和 Agent 映射：

```ini
# 定义模型 profile（名称自定义，如 deepseek、sensenova）
...
LLM_PROFILE_DEEPSEEK_API_KEY=你的DeepSeek_Key
...
# Agent 模型映射（agent_name=profile_name）
LLM_AGENT_MODEL_CODER=deepseek
LLM_AGENT_MODEL_REVIEWER=sensenova
LLM_AGENT_MODEL_TESTER=sensenova
LLM_AGENT_MODEL_ORCHESTRATOR=deepseek
```
...
maker = CoderAgent(model_profile="deepseek")
